...
function loadLayout(path: string) {
...
describe("layouts plugin", () => {
  ...
  describe("front page layout", () => {
    ...
    describe("when theres only one header tag", () => {
      const { body: slide } = parser.parseFromString(
        "<h1>I'm groot</h1>",
        "text/html"
      );
      ...
      it("puts h1 into title placeholder", () => {
        ...
        expect(slide.querySelector("#id58")!.innerHTML.trim()).toBe(
          `<p class="a334" class-names="" cond-style-name="" style="">I'm groot</p>`
        );
      });
