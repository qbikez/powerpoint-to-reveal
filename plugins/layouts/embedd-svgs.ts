...
type Theme = {
  colors: Array<[from: Color, to: Color]>;
};
...
export class Color {
  ...
  public static parse(text: string | null) {
    if (!text) return undefined;
    const rgbMatch =
      /rgb\s*\((?<red>[0-9]+)\s*,\s*(?<green>[0-9]+)\s*,\s*(?<blue>[0-9]+)\s*\)/.exec(
        text
      );
    if (rgbMatch?.groups) {
      return new Color(
        parseInt(rgbMatch.groups["red"]),
        parseInt(rgbMatch.groups["green"]),
        parseInt(rgbMatch.groups["blue"])
      );
    }

    const hexMatch =
      /#(?<red>[0-9a-fA-F]{2})(?<green>[0-9a-fA-F]{2})(?<blue>[0-9a-fA-F]{2})/.exec(
        text
      );
    if (hexMatch?.groups) {
      return new Color(
...
export default class EmbeddSvgsPlugin {
  ...
  constructor(options: Partial<PropType<EmbeddSvgsPlugin, "settings">> = {}) {
    this.settings = {
      ...this.settings,
      ...options,
    };
  ...
  init = async (deck) => {
    ...
    for (let slideIdx = 0; slideIdx < slides.length; slideIdx++) {
      ...
      if (isVerticalParent) continue;

      const theme = slide.dataset.theme || this.settings.defaultTheme;
      const imgs = Array.from(slide.querySelectorAll("img")).filter((i) =>
        i.src?.endsWith(".svg")
      );
  ...
  replaceColor(colorText: string, theme: Theme) {
  ...
  async replaceColors(svg: Element, theme: Theme) {
    Array.from(svg.children).forEach((child) => {
      ...
      const styles = Object.entries((child as HTMLElement).style);
      styles
        .filter(([key, value]) => value && colorAttributes.includes(key))
        .forEach(([key, value]) => {
          (child as HTMLElement).style[key] = this.replaceColor(value, theme);
        });
      this.replaceColors(child, theme);
    });
  }

  async embeddImage(img: HTMLImageElement) {
